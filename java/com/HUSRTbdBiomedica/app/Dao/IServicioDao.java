...
public interface IServicioDao extends CrudRepository<Servicio,Long>{
	...
	@Query("SELECT s from Servicio s order by s.Nombre_servicio")
	public List<Servicio> getAllServicios();
	
	@Query("SELECT s from Servicio s WHERE s.id_Servicio IN (SELECT se.servicio.id_Servicio from SystemEquipo se GROUP BY se.servicio.id_Servicio)")
    ...
    @Query("SELECT e from Equipo e "
    		+ "INNER JOIN Servicio s ON e.servicio.id_Servicio=s.id_Servicio "
    		+ "WHERE e.id_Equipo not in (select hv.equipo.id_Equipo from Hoja_vida hv where hv.Comodato = true) and e.EstadoBaja = false AND e.Periodicidad = 4 AND s.id_Servicio=?1")
    public List<Equipo> findEquiposCuatrimestralservicio(Long id);
    
    @Query("SELECT e from Equipo e "
    		+ "INNER JOIN Servicio s ON e.servicio.id_Servicio=s.id_Servicio "
    		+ "WHERE e.id_Equipo not in (select hv.equipo.id_Equipo from Hoja_vida hv where hv.Comodato = true) and e.EstadoBaja = false AND e.Periodicidad = 3 AND s.id_Servicio=?1")
    public List<Equipo> findEquiposTrimestralservicio(Long id);
    
    @Query("SELECT e from Equipo e "
    		+ "INNER JOIN Servicio s ON e.servicio.id_Servicio=s.id_Servicio "
    		+ "WHERE e.id_Equipo not in (select hv.equipo.id_Equipo from Hoja_vida hv where hv.Comodato = true) and e.EstadoBaja = false AND e.Periodicidad = 2 AND s.id_Servicio=?1")
    public List<Equipo> findEquiposSemestralservicio(Long id);
    
    @Query("SELECT e from Equipo e "
    		+ "INNER JOIN Servicio s ON e.servicio.id_Servicio=s.id_Servicio "
    		+ "WHERE e.id_Equipo not in (select hv.equipo.id_Equipo from Hoja_vida hv where hv.Comodato = true) and e.EstadoBaja = false AND s.id_Servicio=?1 AND e.Periodicidad=1")
    public List<Equipo> findEquiposAnualservicio(Long id);
    
    //SYS
    @Query("SELECT e FROM SystemEquipo e "+
    		"INNER JOIN Servicio s ON e.servicio.id_Servicio=s.id_Servicio "
    		+"WHERE e.EstadoBaja = false AND s.id_Servicio=?1")
    ...
    @Query("SELECT s FROM Servicio s "+
    		...
    		+"GROUP BY s.id_Servicio ORDER BY s.id_Servicio ASC")
    ...
    @Query("SELECT s FROM Servicio s where id_Servicio in(select e.servicio.id_Servicio from Equipo e where e.id_Equipo in (select hv.equipo.id_Equipo from Hoja_vida hv where hv.Comodato = true) and e.EstadoBaja = false)")
